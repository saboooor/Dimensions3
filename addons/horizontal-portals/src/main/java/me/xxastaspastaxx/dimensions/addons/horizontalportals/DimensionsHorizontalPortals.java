		...
		for (double x=min.getX();x<=max.getX();x++) {
			for (double z=min.getZ();z<=max.getZ();z++) {
				...
				if (customPortal.getInsideMaterial().isSolid() || customPortal.getInsideMaterial()==Material.NETHER_PORTAL || customPortal.getInsideMaterial()==Material.END_GATEWAY) {
					entity = new PortalEntitySand(loc, customPortal.getCombinedID(false), customPortal.getInsideSprite() != null);
				} else {
					entity = new PortalEntitySolid(loc, customPortal.getInsideBlockData(false), customPortal.getInsideSprite() != null);
				}
