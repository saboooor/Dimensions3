...
public class ParticlePack {
  ...
  private static HashMap<String, ParticlePack> registered = new HashMap<String, ParticlePack>();
  private HashMap<CompletePortal, ArrayList<ParticleSaved>> ready =
      new HashMap<CompletePortal, ArrayList<ParticleSaved>>();
  ...
  public ParticlePack(String file) {
    ...
    config =
        YamlConfiguration.loadConfiguration(
            new File("./plugins/Dimensions/ParticlePacks/" + file + ".yml"));

    for (String st : config.getStringList("start")) {
      if (st.startsWith("debug")) {
        DimensionsDebbuger.HIGH.print(replaceVars(st.replaceFirst("debug ", ""), null));
      } else if (st.contains("=")) {
        String[] var = st.split(" = ");
        if (var[0].startsWith("global.")) {
          vars.put(var[0].replaceFirst("global.", ""), eval(var[1], null));
        } else {
  ...
  public String replaceVars(String str, HashMap<String, Double> tempVars) {
    if (tempVars != null) {
      for (String var : tempVars.keySet()) {
        if (var.startsWith("global.")) continue;
        str = str.replaceAll("\\b" + var + "\\b", tempVars.get(var) + "");
      }
    }
    for (String var : vars.keySet()) {
      if (var.startsWith("global.")) var = var.replaceFirst("global.", "");
  ...
  public static ArrayList<ParticlePack> load(List<String> stringList) {
    ...
    for (String s : stringList) {
      if (registered.containsKey(s)) {
        res.add(registered.get(s));
      } else {
        ParticlePack pack = new ParticlePack(s);
        res.add(pack);
      }
    }
    return res;
  ...
  public void run(String parent, CompletePortal complete, Location loc) {
    if (ready.get(complete) == null) return;
    ...
    for (String st : config.getStringList(parent + ".play")) {
      ...
      playLine(complete, st, loc, tempVars);
    }

    playLoop(complete, loc, parent + ".for", tempVars);
  }

  private void playLine(
      CompletePortal complete, String st, Location loc, HashMap<String, Double> tempVars) {
    if (ready.get(complete) == null) return;
    if (st.startsWith("debug")) {
      DimensionsDebbuger.HIGH.print(replaceVars(st.replaceFirst("debug ", ""), tempVars));
    } else if (st.startsWith("play")) {
      playParticle(complete, loc, st, tempVars);
    } else if (st.contains("=")) {
      String[] var = st.split(" = ");
      if (var[0].startsWith("global.")) {
        vars.put(var[0].replaceFirst("global.", ""), eval(var[1], tempVars));
      } else {
  ...
  private void playLoop(
      CompletePortal complete, Location loc, String loop, HashMap<String, Double> tempVars) {
    if (ready.get(complete) == null) return;
        ...
        if (key.contains(";")) {
          ...
          while (equals(sp[1], tempVars)) {
            for (String key2 : config.getConfigurationSection(loop + "." + key).getKeys(false)) {
              playLoop(complete, loc, loop + "." + key + "." + key2, tempVars);
              playLoop(complete, loc, loop + "." + key, tempVars);
            }
  ...
  private void playParticle(
      CompletePortal complete, Location loc, String st, HashMap<String, Double> tempVars) {
    String[] spl = st.split(" ");

    if (ready.get(complete) == null) return;
    if (st.contains("DustOptions")) {
      String[] particlesColorString =
          spl[9].replace("DustOptions(Color(", "").replace(")", "").replaceAll(" ", "").split(",");
      Color color =
          Color.fromBGR(
              (int) eval(particlesColorString[2], tempVars),
              (int) eval(particlesColorString[1], tempVars),
              (int) eval(particlesColorString[0], tempVars));

      ready
          .get(complete)
          .add(
              new ParticleSaved(
                  Particle.valueOf(spl[1]),
                  eval(spl[2], tempVars),
                  eval(spl[3], tempVars),
                  eval(spl[4], tempVars),
                  (int) eval(spl[5], tempVars),
                  eval(spl[6], tempVars),
                  eval(spl[7], tempVars),
                  eval(spl[8], tempVars),
  ...
  private boolean equals(String cond, HashMap<String, Double> tempVars) {
    ...
    if (cond.contains("==")) {
      ...
      if (eval(spl[0], tempVars) == eval(spl[1], tempVars)
          && eval(spl[0], tempVars) != Double.MAX_VALUE) return true;
  ...
  public static ParticlePack get(String s) {
    if (registered.containsKey(s)) return registered.get(s);
    else return new ParticlePack(s);
  }

  public void begin(CompletePortal complete) {
    if (!complete.isActive()) return;

    if (!ready.containsKey(complete)) {
      ready.put(complete, new ArrayList<ParticleSaved>());
      try {
        runPortal(complete, complete.getCenter());

        for (PortalEntity en : complete.getPortalEntities()) {
          runTile(complete, en.getLocation());
        }
      } catch (NullPointerException e) {
        ready.remove(complete);
      }
    }
    try {

      ready.get(complete).forEach(par -> par.play());
    } catch (ConcurrentModificationException e) {
      ready.remove(complete);
    }
  }

  public void remove(CompletePortal complete) {
    ready.remove(complete);
  }
