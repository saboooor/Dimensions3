  ...
  public void onIslandRegen(IslandChunkResetEvent e) {
    ...
    DimensionsScheduler.runAsync(
        ...
        new Runnable() {
          ...
          @Override
          public void run() {
            while (!e.getIsland().getLoadedChunks(Environment.NORMAL, true, false).contains(chunk)
                          ...
                          new Runnable() {
                            ...
                            public void run() {
                              for (int X = 0; X < 16; X++) {
                                for (int Z = 0; Z < 16; Z++) {
                                  for (int Y = 1; Y < 255; Y++) {
                                    if (snapshot.getBlockType(X, Y, Z) != Material.OAK_WALL_SIGN)
                                      continue;
