...
public class DimensionsPatreonCosmetics extends DimensionsAddon implements Listener {
  ...
  private HashMap<CompletePortal, ScheduledTask> tasks =
      new HashMap<CompletePortal, ScheduledTask>();
  ...
  public void onDisable() {
    tasks.values().forEach(DimensionsScheduler::cancel);
  }
  ...
  public void postDestroyPortal(CustomPortalBreakEvent e) {
    if (tasks.containsKey(e.getCompletePortal()))
      DimensionsScheduler.cancel(tasks.get(e.getCompletePortal()));
          ...
          public void run() {
            try {
              ...
              try {
                ...
                map.put(
                    "postIgnitePortal", CosmeticEffect.valueOf(mapString.get("postIgnitePortal")));
                map.put(
                    "postDestroyPortal",
