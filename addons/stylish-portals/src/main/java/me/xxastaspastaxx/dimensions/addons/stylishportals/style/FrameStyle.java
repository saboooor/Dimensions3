  ...
  public boolean isPortal(World world, Vector min, Vector max, boolean zAxis) {
    ...
    for (int x = 0; x <= portalWidth; x++) {
      top.add(
          new Location(
                  world,
                  !zAxis ? min.getX() + x : min.getX(),
                  max.getY(),
                  !zAxis ? min.getZ() : min.getZ() + x)
              .getBlock());
    }
    ...
    for (int y = 1; y < portalHeight; y++) {
      side1.add(
          new Location(
                  world,
                  !zAxis ? min.getX() : min.getX(),
                  min.getY() + y,
                  !zAxis ? min.getZ() : min.getZ())
              .getBlock());
      side2.add(
          new Location(
                  world,
                  !zAxis ? max.getX() : max.getX(),
                  min.getY() + y,
                  !zAxis ? max.getZ() : max.getZ())
              .getBlock());
    }

    return (patterns[1].isPattern(side1.toArray(new Block[0]))
            && patterns[2].isPattern(side2.toArray(new Block[0])))
        || (patterns[1].isPattern(side2.toArray(new Block[0]))
            && patterns[2].isPattern(side1.toArray(new Block[0])));
  }
  ...
  public void setPortal(Location newLocation, World world, Vector min, Vector max, boolean zAxis) {
    ...
    for (int x = 0; x <= portalWidth; x++) {
      top.add(
          new Location(
                  world,
                  !zAxis ? newLocation.getX() + x : newLocation.getX(),
                  newLocation.getY() + portalHeight,
                  !zAxis ? newLocation.getZ() : newLocation.getZ() + x)
              .getBlock());
    }
    ...
    for (int y = 1; y < portalHeight; y++) {
      side1.add(
          new Location(world, newLocation.getX(), newLocation.getY() + y, newLocation.getZ())
              .getBlock());
      side2.add(
          new Location(
                  world,
                  !zAxis ? newLocation.getX() + portalWidth : newLocation.getX(),
                  newLocation.getY() + y,
                  !zAxis ? newLocation.getZ() : newLocation.getZ() + portalWidth)
              .getBlock());
    }
