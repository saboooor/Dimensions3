...
public class SidePattern {
  ...
  public boolean isPattern(Block[] blocks) {
    ...
    while (blocksIndex < max) {
      for (int i = Math.min(1, patterns.size() - 1);
          i < Math.max(patterns.size() - 1, Math.min(1, patterns.size() - 1) + 1);
          i++) {
  ...
  public void setPattern(Block[] blocks, boolean zAxis) {
    ...
    patterns.get(0).placeStart(blocks, zAxis);
    patterns.get(patterns.size() - 1).placeEnd(blocks, zAxis);
    ...
    while (blocksIndex < max) {
      for (int i = Math.min(1, patterns.size() - 1);
          i < Math.max(patterns.size() - 1, Math.min(1, patterns.size() - 1) + 1);
          i++) {
        patterns.get(i).placeStart(blocksIndex, blocks, zAxis);
        blocksIndex += patterns.get(i).getSize();
      }
    }
  }
