...
public class WorldGuardFlags {
  ...
  public WorldGuardFlags() {
    ...
    try {
      ...
      if (igniteExisting instanceof StateFlag) {
        ...
        System.out.println(
            "Some other plugin is using dimensions' custom flags. This could cause errors. Please"
                + " remove this plugin.");
      }
  ...
  public boolean testState(Player p, Location loc, Object flag) {
    ...
    return ignite == State.ALLOW
        || worldGuard.getSessionManager().hasBypass(localPlayer, localPlayer.getWorld());
  }
}
