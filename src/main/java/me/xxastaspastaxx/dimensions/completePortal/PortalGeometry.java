...
public class PortalGeometry {
  ...
  private static HashMap<CustomPortal, PortalGeometry> customGeometry =
      new HashMap<CustomPortal, PortalGeometry>();
  ...
  public byte getPortalWidth(CustomPortal customPortal) {
    return (byte)
        (customPortal.getFixedExitPortalWidth() == -1
            ? portalWidth
            : customPortal.getFixedExitPortalWidth());
  }
  ...
  public byte getPortalHeight(CustomPortal customPortal) {
    return (byte)
        (customPortal.getFixedExitPortalHeight() == -1
            ? portalHeight
            : customPortal.getFixedExitPortalHeight());
  }
  ...
  public PortalGeometry getPortal(CustomPortal customPortal, Location loc, boolean zAxis) {
    ...
    if (!customPortal.isPortalBlock(minLocation.getBlock())
        || !customPortal.isPortalBlock(maxLocation.getBlock())) return null;
  ...
  public boolean isInside(Location location, boolean outside, boolean corner) {
    ...
    if (outside && !corner) {
      ...
      if (min.getY() == location.getY() || max.getY() == location.getY()) {
        ...
        if (!zAxis && (min.getX() == location.getX() || max.getX() == location.getX()))
          return false;
      }
    }
  ...
  public void buildPortal(Location newLocation, World destinationWorld, CustomPortal customPortal) {
    ...
    double maxSide =
        ((zAxis ? newLocation.getZ() : newLocation.getX()) + getPortalWidth(customPortal));
