...
public class CustomPortal {
  ...
  private ArrayList<DimensionsAddonPlayerGUIAction> playerGUIActions =
      new ArrayList<DimensionsAddonPlayerGUIAction>();
  ...
  public CustomPortal(
      ...
      int fixedExitPortalWidth,
      int fixedExitPortalHeight,
      List<String> allowedWorldsList,
      int teleportDelay,
  ...
  public CompletePortal tryIgnite(Player player, ItemStack item, Location loc) {
    ...
    return Dimensions.getCompletePortalManager()
        .createNew(
            new CompletePortal(this, loc.getWorld(), temp),
            player,
            CustomPortalIgniteCause.PLAYER,
            item);
  }
  ...
  public void setInsideBlockData(BlockData blockData) {
    insideBlockData =
        new BlockData[] {
          CustomPortalLoader.getInsideBlockData(false, blockData.clone()),
          CustomPortalLoader.getInsideBlockData(true, blockData.clone())
        };
  ...
  public EntityType getNextSpawn() {
    return entitySpawnList.keySet().stream()
        .filter(type -> entitySpawnList.get(type) >= DimensionsUtils.getRandom(0, 100))
        .findAny()
        .orElse(null);
  }
